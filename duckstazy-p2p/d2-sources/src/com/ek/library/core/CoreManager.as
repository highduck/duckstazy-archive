package com.ek.library.core
{
	...
	public class CoreManager 
	{
		...
		//static private var _eventFPS:CoreEvent;
		//static private var _eventSlowTick:CoreEvent;
		//static private var _eventTick:CoreEvent;
		...
		static private function enterFrameHandler(event:Event):void
		{
			...
			if ( ( now - _fpsLastMeas ) > 1000 )
			{
				...
				//if(!_eventFPS)
					//_eventFPS = new CoreEvent(CoreEvent.FPS);
			...
			if(_intervalTimer > _intervalRate)
			{
				...
				//if(!_eventSlowTick)
					//_eventSlowTick = new CoreEvent(CoreEvent.SLOW_TICK);
					
				_eventDispatcher.dispatchEvent(new CoreEvent(CoreEvent.SLOW_TICK));
			}
			
			//if(!_eventTick)
				//_eventTick = new CoreEvent(CoreEvent.TICK);
...
import com.ek.library.core.CoreEvent;
...
class RootGameObject extends GameObject 
{
	...
	private function onSlowTick(e:Event):void
	{
		slowTick();
	}
}
