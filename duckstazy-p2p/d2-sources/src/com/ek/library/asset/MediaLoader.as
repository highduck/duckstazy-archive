package com.ek.library.asset
{
	...
	import com.ek.library.utils.VisualUtil;
	...
	{
		public function MediaLoader(url:String, type:String, antiCache:String = null, retryCount:int = 5, global:Boolean = false)
		{
			switch(type)
			{
			...
			super(url, type, antiCache, retryCount, global);
		}
		...
		protected override function _initializeLoader():void
		{
			...
			if(_type == LoaderType.IMAGE) {
				_loaderContext = new LoaderContext(true, appDomain);
			}
			else {
				switch(Security.sandboxType)
				{
					case Security.REMOTE:
						_loaderContext = new LoaderContext(false, appDomain, SecurityDomain.currentDomain);
						break;
					case Security.LOCAL_TRUSTED:
					case Security.LOCAL_WITH_FILE:
					case Security.LOCAL_WITH_NETWORK:
						_loaderContext = new LoaderContext(false, appDomain);
						break;
				}
			}
		}
		...
		protected override function _resolveContent():void
		{
			switch(_type)
			{
				...
				case LoaderType.SWF:
					...
					VisualUtil.stop(_loader as Loader);
					break;
			}
		}
