package com.ek.library.asset
{
	import com.ek.library.debug.LogLevel;
	...
	public class AssetManager
	{
		...
		private static function createLoader(url:String, type:String = null, antiCache:Boolean = true, global:Boolean = false):BaseLoader
		{
			var loader:BaseLoader;
			var ac:String;
			...
			if(_typesLoader.hasOwnProperty(type))
			{
				...
				loader = ( new (_typesLoader[type])(url, type, ac, _retryCount, global) ) as BaseLoader;
			}
		...
		private static function addItem(item:AssetItem, ignoreBaseURL:Boolean, caseSensitive:Boolean):void
		{
			...
			if(!loader)	loader = addLoader(item.url, item.type, item.antiCache, item.global);
		...
		private static function addItemFromXML(xml:XML):void
		{
			...
			if(xml.hasOwnProperty("@global"))
				item.global = xml.@global;
		...
		private static function addLoader(url:String, type:String = null, antiCache:Boolean = true, global:Boolean = false):BaseLoader
		{
			var loader:BaseLoader = createLoader(url, type, antiCache, global);
